...
export default function NewItem() {
    ...
    return(
        <main className="flex justify-center w-max p-2 m-4 bg-gray-900 max-w-sm mx-auto rounded">
            <form onSubmit={handleSubmit}>
                <h2 className="text-xl text-white pb-5">Add New Item</h2>
                <div className="text-white mb-3">
                    <label className="" ></label>
                    <input type="text" value={name} onChange={handleSetName} required placeholder="Item Name" className="rounded text-black" />
                </div>
                <div className="mb-3 flex justify-between">
                    <input type="number" value={quantity} onChange={handleSetQuantity} min="1" max="99" required className="rounded w-14"/>
                    <label></label>
